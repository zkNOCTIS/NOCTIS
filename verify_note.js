...
async function main() {
  ...
  // Compute spendingKeyHash = hash(spendingKey)
  const spendingKeyHash = poseidon.F.toString(poseidon([BigInt(spendingKey)]));
  console.log('spendingKeyHash:', spendingKeyHash);
  
  // Compute commitment = hash(spendingKeyHash, balance, randomness)
